    ...
    GenerateRandom ->
      ...
      , Random.generate
        ...
        ( Random.Array.array model.worldLength.y <|
          ...
          Random.map (\n -> n < model.density) <| Random.int 0 99
        )
      )
    Reset newCells ->
      ( { model |
        ...
        (
          Array.indexedMap
          (
            \yIndex xCells -> Array.indexedMap (
              \xIndex cell ->
                if xIndex >= model.margin &&
                  xIndex < model.worldLength.x - model.margin &&
                  yIndex >= model.margin &&
                  yIndex < model.worldLength.y - model.margin then
                  cell
                else False
            ) xCells
          )
  ...
  , Html.div []
    [ Svg.svg
      ...
      (
        ...
        (
          List.concat <| List.map Array.toList <| Array.toList <|
            Array.indexedMap
            (
              \yIndex xCells -> Array.indexedMap
                (\xIndex cell -> toSvg { x = xIndex, y = yIndex } cell model.cellLook model.colorScheme) xCells
            )
            model.cells
        )
      )
